	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			task =  new AsyncTask<ArrayList<Integer>, Integer, ArrayList<LatLng>>() {
				protected ArrayList<LatLng> doInBackground(ArrayList<Integer>... loc) {
			     ...
			     protected void onPostExecute(ArrayList<LatLng> result) {
			    	 ...
			    	 for (int i = 0; i < result.size(); i++) {
			    		if (RouteResultsActivity.coordinatesType.containsKey(i)) {
			    			...
			    			ArrayList<Object> array=RouteResultsActivity.coordinatesType.get(i);
			    	 ...
			    	 if (line.getPoints().size()>1) gmap.addPolyline(line);
			    	...
			    	mapFragment.getView().post(new Runnable() {
						...
						public void run() {
							...
							gMap.animateCamera(CameraUpdateFactory.newLatLngBounds(getBounds(), (int) px));
						}
			    	});
			     }
			};

...
public class RouteResultsActivity extends Activity implements
	...
	private void initialiseRouteList(ListView listview) {
		...
		SimpleAdapter adapter = new SimpleAdapter(this, adapter_list,
				R.layout.route_item, new String[] { "departure", "duration",
						"arrival", "icons" },
						new int[] { R.id.depart_textview, 
									R.id.duration_textview,
									R.id.arrive_textview,
									R.id.changes_layout});
	...
	public void showRouteDetails(int current_plan_number) {
		...
		for (PartialRoute proute : route.getPartials()) {
			...
			m.put("duration",
					"Transit time: "
							+ Integer.toString(proute.getMinutes())
							+ " minutes");
			m.put("directions", proute.getDirections());
			m.put("icon", proute.getIcon());
			if (i==0) m.put("type","first");
			else m.put("type","intermediate");
			PartialRouteType type=proute.getMeansOfTransportBareType();
			String stringType;
			if (type!=PartialRouteType.TUBE) stringType=proute.getMeansOfTransportType(); 
			else stringType=proute.getMeansOfTransportShortName();
		...
		SimpleAdapter adapter = new SimpleAdapter(this, adapter_list,
				R.layout.partial_route_item, new String[] { "fromName",
						"fromTime", "directions", "duration",
						"type","motType","motType","motType",
						"icon"},
				new int[] { R.id.fromname_textview, R.id.fromtime_textview,
						R.id.directions_textview, R.id.duration_textview,
						R.id.images_layout, R.id.start_imageview, R.id.intermediate_imageview,R.id.destination_imageview,
						R.id.mot_imageview});
